...
func (r *PluginRepository) BatchUpsertChapters(docID int, source string, global bool, chapters []models.Chapter) bool {
	var doc models.Document
	...
	for i := range chapters {
		chapters[i].DocumentID = uint(docID)
	}

	err := r.DB.Clauses(clause.OnConflict{
		Columns:   []clause.Column{{Name: "document_id"}, {Name: "url"}},
		UpdateAll: true,
	}).CreateInBatches(chapters, 100).Error
...
func (r *PluginRepository) GetChapters(docID int, source string, global bool) ([]models.Chapter, bool) {
	...
	r.DB.Where("document_id = ?", docID).Order("order_val ASC").Find(&chapters)
