...
Lector plugins use Lua to extend the application. Plugins can be installed as either single `.lua` files or `.zip` archives. Lector automatically organizes all plugins into dedicated subdirectories (e.g., `plugins/my-plugin/init.lua`).
...
Programmatically defines the plugin's unique identifier. This ID is used as the directory name in `plugins/` and as the RPC target name. It should be lowercase and contain no spaces.
